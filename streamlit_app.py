...
def file_recognizer(audio_video_file):
    ...
    with audio_video_file as source:
        ...
        try:
            ...
            st.markdown("Could not request results from speech recognition service; {0}".format(e))
    if flag:
        output_options()
...
file = st.file_uploader('Upload a .wav or a video file to output the corresponding transcription')
if file:
...
if not st.session_state['run'] and st.session_state['text']:
    if language == 'en-US':
        st.session_state['text'] = st.session_state['text'][0].upper()+st.session_state['text'][1:-1]+'.'
    else:
